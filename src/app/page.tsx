...
const App = () => {
  ...
  /* ================= CONTENT ================= */
  const services = [
    { title: lang === 'en' ? 'Full-Stack Engineering' : 'វិស្វកម្ម Full-Stack', description: lang === 'en' ? 'Enterprise applications using Angular, .NET Core, Docker.' : 'កម្មវិធីសហគ្រាសដោយ Angular, .NET Core និង Docker។', icon: '💻' },
    { title: lang === 'en' ? 'SMM Panel Specialist' : 'អ្នកជំនាញ SMM', description: lang === 'en' ? 'High-performance SMM platforms with KHQR & API automation.' : 'ប្រព័ន្ធ SMM ជាមួយ KHQR និង API ស្វ័យប្រវត្តិ។', icon: '🚀' },
    { title: lang === 'en' ? 'Digital Controller' : 'អ្នកគ្រប់គ្រងឌីជីថល', description: lang === 'en' ? 'SEO, paid ads, and brand growth strategy.' : 'យុទ្ធសាស្ត្រ SEO និង Ads សម្រាប់ការលូតលាស់ម៉ាក។', icon: '📈' },
    { title: lang === 'en' ? 'Creative Producer' : 'អ្នកផលិតមាតិកា', description: lang === 'en' ? 'High-engagement video & storytelling.' : 'មាតិកាវីដេអូ និងការនិទានជំនាញ។', icon: '🎬' },
  ];
  ...
  const platforms = [
    { name: 'SMM Panel System', desc: lang === 'en' ? 'High-performance SMM panel with automation & payment integration.' : 'ប្រព័ន្ធ SMM ជាមួយការទូទាត់ និងស្វ័យប្រវត្តិ។', icon: '⚙️' },
    { name: 'Digital Marketing Dashboard', desc: lang === 'en' ? 'Campaign tracking, analytics & ROI control.' : 'គ្រប់គ្រងយុទ្ធសាស្ត្រ និងវិភាគទិន្នន័យ។', icon: '📊' },
    { name: 'Enterprise Web Apps', desc: lang === 'en' ? 'Custom internal systems for business operations.' : 'ប្រព័ន្ធផ្ទៃក្នុងសម្រាប់អាជីវកម្ម។', icon: '🏢' },
    { name: 'Payment Integration', desc: lang === 'en' ? 'KHQR, ABA, API-based payment solutions.' : 'ប្រព័ន្ធទូទាត់ KHQR និង ABA។', icon: '💳' },
  ];
  ...
  const socials = [
    { name: 'Github', link: 'https://github.com/khoem168', icon: '🐱' },
    { name: 'LinkedIn', link: 'https://linkedin.com/in/khoem168', icon: '💼' },
    { name: 'Facebook', link: 'https://facebook.com/heang.chhengkhoem.me', icon: '📘' },
    { name: 'Instagram', link: 'https://instagram.com/khoem168', icon: '📸' },
    { name: 'Twitter', link: 'https://x.com/khoem168', icon: '🐦' },
  ];
    ...
    <main className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <nav className="fixed top-0 w-full z-50 bg-background/70 backdrop-blur-xl border-b border-gray-500/10">
        ...
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex gap-2">
            <button onClick={toggleLang} className="text-[10px] font-black border px-3 py-1 rounded hover:bg-gold hover:text-black transition">
              {lang === 'en' ? 'KH' : 'EN'}
            </button>
            <button onClick={toggleTheme} className="p-2 rounded-full border hover:bg-gray-500/10 transition">
              {theme === 'dark' ? '☀️' : '🌙'}
            </button>
          </div>
        </div>
      ...
      <motion.section ref={sections.skills} className="py-32" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeInUp}>
        <div className="container mx-auto px-6 max-w-3xl">
          <h3 className="text-5xl font-serif italic text-gold mb-12 text-center">{lang === 'en' ? 'Technical Skills' : 'ជំនាញបច្ចេកទេស'}</h3>
          <div className="space-y-6">
            {skills.map((s, i) => (
              <div key={s.name}>
      ...
      <motion.section ref={sections.contact} className="py-48 text-center" initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeInUp}>
        <h3 className="text-6xl font-serif italic text-gold mb-12">{lang === 'en' ? 'Start a New Project' : 'ចាប់ផ្តើមគម្រោងថ្មី'}</h3>
        <a href="mailto:heang.chhengkhoem.me@gmail.com" className="inline-block px-16 py-6 bg-foreground text-background rounded-full font-black tracking-[0.35em] text-[10px] hover:scale-105 transition">
          {t.emailCta}
        </a>
      </motion.section>
